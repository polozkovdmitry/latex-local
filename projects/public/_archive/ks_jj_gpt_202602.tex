...
Langevin dynamics, originally introduced in \cite{Langevin1908}, are applied in modern science across a wide range of fields, including fluid dynamics, biology, economics, and social science \cite{Naldi2010,Pareschi2013}, and more recently in artificial intelligence \cite{Cheng2018}.

\section{Clarification on Pavliotis's scaling and limits}

(This section mainly refers to Chapter 6 in \cite{Pavliotis2014}.) The Langevin model in \cite{Pavliotis2014} (see Equation (6.1), p. 181) corresponds to a second-order stochastic system describing the motion of a “particle” with 
\[ 
dq_t = -\,\nabla U(q_t)\,dt \;-\; \eta\,\dot{q}_t\,dt \;+\; \sqrt{\frac{2\eta}{\beta}}\,dW_t, \qquad t \ge 0, 
\] 
where $q = (q_t)_{t\ge0}$ denotes the position process of the particle and $\dot{q} = (\dot{q}_t)_{t\ge0}$ the associated velocity process. Motions are assumed to start at the position $q_0$ and velocity $\dot{q}_0$. (For notational consistency, the potential force and friction coefficient are denoted here by $\nabla U$ and $\eta$ instead of $\nabla V$ and $\gamma$ as in \cite{Pavliotis2014}.) To this model, we associate the family of rescaled dynamics 
\[ 
...
While the Kramers--Smoluchowski limit is more directly linked to \eqref{eq:3.6}, this proposition is purposely written to emphasize the successive “easy” part \eqref{eq:3.4}\footnote{Stated without proof details in \cite{Pavliotis2014}, p. 206.} and the more “tricky” part \eqref{eq:3.6} of the limit. 
...
\tag{3.7}
\] 
where $|\nabla f|_{\text{Lip}} := \sup_{x \neq y}\frac{\|\nabla f(x) - \nabla f(y)\|}{\|x - y\|}$ is the Lipschitz constant of $\nabla f$. Consequently (after some standard calculations on the SDE \eqref{eq:3.5}, see e.g. \cite{Karatzas1991}), we obtain that for all integers $p \ge 1$, 
\[ 
...
It follows that 
\[ 
V_t^\gamma = e^{-\gamma^{-1}t}\,U_t = e^{-\gamma^{-1}t}V_0 + \gamma^{-1}\int_{0}^{\,t} e^{\gamma^{-1}(s - t)}\,\nabla f(X_s^\gamma)\,ds + \sigma\,\gamma^{-1}\int_{0}^{\,t} e^{\gamma^{-1}(s - t)}\,dW_s\,,
\tag{3.9}
\] 
and, further, 
\begin{align}
...
\medskip\noindent \textit{Proof of \eqref{eq:3.6}:} Before establishing \eqref{eq:3.6}, let us point out that, in contrast to some approaches (e.g. \cite{Freidlin2004}), our proof does \emph{not} rely on a coupling argument. Instead, we leverage an exponential martingale and maximal inequalities. Define 
\[ 
...
it holds that 
\[ 
f(t,X_t) = f(0,X_0) + \int_{0}^{\,t} \partial_s f(s, X_s)\,ds + \sum_{i=1}^d \int_{0}^{\,t} \partial_{x_i} f(s,X_s)\,dX_s^i + \frac{1}{2}\sum_{i,j=1}^d \int_{0}^{\,t} \partial^2_{x_i x_j} f(s,X_s)\,d\langle X^i, X^j \rangle_s\,.
\] 
...
\[ 
f(t,X_t) = f(0,X_0) + \int_{0}^{\,t} \partial_s f(s,X_s)\,ds + \int_{0}^{\,t} \nabla_x f(s,X_s)\cdot dM_s + \int_{0}^{\,t} \nabla_x f(s,X_s)\cdot dA_s + \frac{1}{2} \int_{0}^{\,t} \text{Tr}\!\Big[\nabla_x^2 f(s,X_s)\,d\langle X,X\rangle_s\Big]\,.
\] 
